...
const expected = ["Palace","Bobby the Breadasaurus","Evil Doom Girl Adventures","Thumb Command","Hearts","Spades","Euchre"];
...
if (doomState.h1 !== "Evil Doom Girl Adventures" || !/in development/i.test(doomState.status) || !doomState.background.includes("78, 42, 132") || doomState.falseClaim) failures.push("Evil Doom Girl Adventures title, status, purple, or honesty gate failed");
if (!["Run","Jump","Duck","Slide","Dash","Climb","Rope","Swing"].every((move) => doomState.moves.includes(move))) failures.push("Evil Doom Girl Adventures movement gate failed");
