...
for (const route of responsiveRoutes) {
  for (const [width, height] of viewports) {
    ...
    if ([390, 768, 1366].includes(width)) await page.screenshot({ path: resolve(evidence, `${route.replace(".html", "")}-${width}x${height}.png`), fullPage: true });
    await page.close();
  }
}
...
for (const [from, to] of redirects) {
  ...
  if (raw.status() !== 200 || !body.includes(`data-route-target="${to}"`) || !body.includes(`https://4ohi.com/${to}`) || !finalUrl.includes(`/${to}?source=legacy#mission`)) fail(`${from}: compatibility redirect failed`);
  await page.close();
}

await browser.close();
writeFileSync(resolve(evidence, "acceptance-results.json"), `${JSON.stringify(results, null, 2)}\n`);
if (failures.length) {
  console.error(`Company architecture QA failed with ${failures.length} issue(s):`);
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}
console.log(`Company architecture QA passed: ${regularRoutes.length} routes, ${responsiveRoutes.length * viewports.length} responsive renders, five Thumb Command articles, global navigation, reduced motion, and four compatibility redirects.`);
