...
const out = resolve(process.env.PALACE_OUT || "docs/visual-evidence/palace-web-2026-08-31");
...
if (!process.env.SITE_URL) await new Promise((resolveListen) => server.listen(4217, "127.0.0.1", resolveListen));
...
await test("In-progress game resumes after tab closure", async () => { const { context, page } = await open(); await page.evaluate(() => window.__PALACE_WEB_QA__?.reset()); await page.getByRole("button", { name:"Deal the cards" }).click(); await page.waitForSelector(".palace-app-frame.is-setup"); await page.close(); const reopened = await context.newPage(); await reopened.goto(`${base}/palace-play.html?palaceQa=1&fast=1&seed=5`, { waitUntil:"networkidle" }); await reopened.waitForSelector(".palace-app-frame.is-setup"); ok((await reopened.evaluate(() => JSON.parse(localStorage.getItem("4oh_palace_web_v1")).status)) === "inProgress", "closed tab did not resume"); await context.close(); });await test("Keyboard operation, dialog escape, and visible focus", async () => { const { context, page } = await open(); await page.evaluate(() => window.__PALACE_WEB_QA__?.reset()); await page.getByRole("button", { name:"Deal the cards" }).focus(); await page.keyboard.press("Enter"); await page.getByRole("button", { name:"Start hand" }).focus(); ok(await page.getByRole("button", { name:"Start hand" }).evaluate((node) => getComputedStyle(node).outlineStyle !== "none"), "focus ring missing"); await page.keyboard.press("Enter"); await page.getByRole("button", { name:"View log" }).click(); ok(await page.getByRole("dialog").isVisible(), "log closed"); await page.keyboard.press("Escape"); ok(!(await page.getByRole("dialog").isVisible()), "escape did not dismiss"); await context.close(); });
...
await test("One complete game through rendered public controls", async () => {
  ...
  const afterSwap = await page.evaluate(() => JSON.parse(localStorage.getItem("4oh_palace_web_v1")).game.message);
  ...
  await page.waitForSelector(".palace-app-frame.is-setup");
  ok((await page.evaluate(() => JSON.parse(localStorage.getItem("4oh_palace_web_v1")).status)) === "inProgress", "setup did not resume");
  ...
  while (!(await page.locator(".palace-complete").count()) && actions < 4000) {
    const snapshot = await page.evaluate(() => JSON.parse(localStorage.getItem("4oh_palace_web_v1")).game);
    if (!snapshot) break;
    ...
    } else await page.waitForTimeout(process.env.SITE_URL ? 100 : 12);
    actions += 1;
  }
  ...
  const completed = await page.evaluate(() => JSON.parse(localStorage.getItem("4oh_palace_web_v1")));
